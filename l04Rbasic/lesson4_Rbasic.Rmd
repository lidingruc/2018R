...
cbind(data,ave2) # data <- cbind(data,ave2) 
```
...
在R语言中，所有操作和运算都是由函数完成的。和数学中的函数类似，大多数的函数由函数名和参数组成的。

例如下面计算指数的例子中，`exp`是函数名，括号中的1则是参数。又可以分为三部分构成，即输入、运算、输出。
```{r}
exp(1)
exp(c(1,2,3,4))
```
...
函数在调用时会新建一个特殊的子环境，用以处理函数中涉及到的变量，例如上面函数中的area变量，这种变量称为__局部变量__。因为不会在全局环境中出现而影响其它函数，使用起来非常安全。

函数内部可以调用__全局变量__，例如上面的pi。


## 变量
R语言中可以给标量、向量、数据框、函数等等指定一个“名字“，反过来，这相对于给这个名字对应变量赋值。赋值操作符为等号“`=`”，也可以是"`<-`"。下面的例子就是先将向量赋值给x变量，然后求取其对数。
```{r}
x <- c(1,2,3,4)
log(x)
```

如果我们想从变量x（目前为一个集合）中取出某一个元素的话，可以使用方括号加索引号：
```{r eval=FALSE}
x[2] # 第二个元素
x[2:4] # 第2到4个元素
x[-4] # 减去第4个元素后
